...
export const MarketBar = ({market}: {market: string}) => {
    ...
    useEffect(() => {
        SignalingManager.getInstance().registerCallback("ticker", (data: Partial<TickerData>)  =>  setTicker(prevTicker => ({
    ...
    return <div>
        <div className="flex items-center flex-row relative w-full border-b">
            <div className="flex items-center justify-between flex-row pr-4">
...
function Ticker({market}: {market: string}) {
    return <div className="flex h-[60px] shrink-0 space-x-4">
        <div className="flex flex-row relative ml-2 -mr-4">
            <Image  alt="SOL Logo" loading="lazy" decoding="async" data-nimg="1" className="z-10 rounded-full h-6 w-6 mt-4 outline-baseBackgroundL1"  src="/sol.webp" width={24} height={24} />
            <Image  alt="USDC Logo" loading="lazy"decoding="async" data-nimg="1" className="h-6 w-6 -ml-2 mt-4 rounded-full" src="/usdc.webp" width={24} height={24}/>
        </div>
